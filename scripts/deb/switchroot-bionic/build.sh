...
export CMAKE_PREFIX_PATH=/opt/TsVitchPE
...
cd /opt
mkdir -p /tmp/deb/DEBIAN /tmp/deb/usr /tmp/deb/opt/TsVitchPE/lib
...
cp -d /opt/TsVitchPE/lib/*.so.* /tmp/deb/opt/TsVitchPE/lib
mv /tmp/deb/opt/TsVitchPE/share /tmp/deb/usr
sed -i 's|Exec=TsVitchPE|Exec=/opt/TsVitchPE/bin/TsVitchPE|' /tmp/deb/usr/share/applications/it.belloworld.TsVitchPE.desktop
cp scripts/deb/switchroot-bionic/control /tmp/deb/DEBIAN
dpkg --build /tmp/deb TsVitchPE-Linux-aarch64-switchroot-ubuntu.deb
